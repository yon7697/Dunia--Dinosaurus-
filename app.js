...
let qs=[],qi=0,sc=0;
function startQuiz(){
const sh=[...dinos].sort(()=>Math.random()-.5).slice(0,5);
qs=sh.map(d=>{
const wr=dinos.filter(x=>x.name!==d.name).sort(()=>Math.random()-.5).slice(0,3);
const op=[...wr,d].sort(()=>Math.random()-.5);
return{q:`Siapakah aku? ${d.fact}`,ans:d.name,op:op.map(o=>o.name)};
});
qi=0;sc=0;renderQuiz();
}
function renderQuiz(){
const box=document.getElementById('quizBox');
if(qi>=qs.length){
box.innerHTML=`<h2>Kuis Selesai! 🎉</h2><div class="result">Skor: ${sc} / ${qs.length}</div><p>${sc===qs.length?'Luar biasa! Kamu ahli dinosaurus! 🌟':'Ayo coba lagi ya! 💪'}</p><button class="btn-restart" onclick="startQuiz()">🔄 Main Lagi</button>`;
return;
}
const q=qs[qi];
box.innerHTML=`<h2>Soal ${qi+1} dari ${qs.length}</h2><div class="q-text">${q.q}</div><div class="q-opts">${q.op.map(o=>`<button onclick="answer('${o}')">${o}</button>`).join('')}</div>`;
}
function answer(o){
if(o===qs[qi].ans){sc++;speak('Benar!');}
else{speak('Salah, coba lagi ya');}
qi++;setTimeout(renderQuiz,400);
}
